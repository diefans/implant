...
@pytest.mark.parametrize('con_str, params', [
...
def test_parse_connection_string(con_str, params):
    from implant import connect

    r = connect.ConnectorParams.parse(con_str)
    assert r == params
...
def test_parse_connection_string(con_str, params):
    ...
    params = connect.ConnectorParams(*params)
    assert params.unparse() == con_str


def test_connector_params_create():
    from implant import connect

    p = connect.ConnectorParams.create(connect.Local())
